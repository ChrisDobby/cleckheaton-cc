...
const Fixture = ({ fixture }: { fixture: Fixture }) => (
  <article className="fixture">
    <h4 className="fixture-date-team">
      <span className="fixture-date">{format(new Date(fixture.matchDate), 'dd-MMM (HH:mm)')}</span>
...
const UpcomingFixtures = ({ fixtures, swipeable }: Props) => {
  ...
  const fixturesList = fixtures.map(fixture => <Fixture key={fixture._id} fixture={fixture} />);

  const fixturesContainerContent = swipeable ? <SwipeableViews style={{ padding: '0 30px' }}>{fixturesList}</SwipeableViews> : fixturesList;
